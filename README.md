...
bce-tool is a Rust implementation of a codebase context engine that enables AI assistants to search and understand codebases using natural language queries. It provides:
...
The easiest way to install and run bce-tool is via npx:

```bash
npx bce-tool --base-url <API_URL> --token <AUTH_TOKEN>
```
...
git clone https://github.com/linqiu919/bce-tool-rs.git
cd bce-tool

# Build release binary
cargo build --release

# The binary will be at target/release/bce-tool
```
...
| `--allow-non-git` | Allow indexing directories that are not inside a Git repository. By default, indexing/search/enhance refuse non-Git directories to avoid accidentally indexing personal folders (e.g. Downloads) |
...
| `--search <query>` | One-shot search mode: index the current directory, run codebase retrieval for the query, print the result to stdout, then exit (no MCP server). Useful for scripts and agent skills |
...
| `PROMPT_ENHANCER_ENDPOINT` | Endpoint selection: `new` (default), `old`, `claude`, `openai`, `gemini`, or `codex` (also reads `BCE_ENHANCER_ENDPOINT` as fallback) |
...
RUST_LOG=debug bce-tool --base-url https://api.example.com --token your-token-here
```
...
```bash
bce-tool --base-url https://api.example.com --token your-token-here --transport lsp
```
...
```toml
[mcp_servers.bce-tool]
command = "npx"
args = ["bce-tool", "--base-url", "https://api.example.com", "--token", "your-token-here", "--transport", "lsp"]
env = { RUST_LOG = "info" }
...
```bash
claude mcp add-json bce-tool --scope user '{"type":"stdio","command":"npx","args":["bce-tool","--base-url","https://api.example.com/","--token","your-token-here"],"env":{}}'
```
...
$ cat settings.local.json
{
  "permissions": {
    "allow": [
      "mcp__bce-tool__search_context",
      "mcp__bce-tool__enhance_prompt"
    ]
  }
}
```
...
- Start bce-tool with `--no-webbrowser-enhance-prompt`.
...
The tool supports multiple backend endpoints, controlled by the `PROMPT_ENHANCER_ENDPOINT` environment variable (with `BCE_ENHANCER_ENDPOINT` as a backward-compatible fallback):
...
bce-tool --enhance-prompt "Refactor authentication logic"
```
...
You can customize file filtering by creating a `.bceignore` file in your project root. It uses the same syntax as `.gitignore`:
...
Both `.gitignore` and `.bceignore` patterns are merged, with `.bceignore` taking precedence in case of conflicts.
...
bce-tool --base-url ... --token ... --upload-concurrency 4

# Fixed timeout, adaptive concurrency
bce-tool --base-url ... --token ... --upload-timeout 60

# Disable adaptive entirely (use static heuristic)
bce-tool --base-url ... --token ... --no-adaptive
```
...
- Only processes the root `.gitignore` and `.bceignore` files (nested ignore files are not supported)
...
This project is a fork of [ace-tool-rs](https://github.com/missdeer/ace-tool-rs) by [missdeer](https://github.com/missdeer), adapted as the `bce-tool` client for [Better Context Engine](https://github.com/linqiu919/better-context-engine). It inherits the upstream dual-license terms:

- **Non-commercial / personal use** — [GPL v3](LICENSE)
