...
use bce_tool::index::{Blob, FileEntry, IndexData, IndexManager, IndexResult, IndexStats};
...
fn test_collect_files_with_text_files() {
    ...
    // Check that the expected files are included (may include .gitignore from get_bce_dir)
...
fn test_load_index_corrupted_data_returns_empty() {
    ...
    let bce_dir = temp_dir.path().join(".bce-tool");
    fs::create_dir_all(&bce_dir).unwrap();
    let index_path = bce_dir.join("index.bin");
...
fn test_save_index_creates_file() {
    ...
    let index_path = temp_dir.path().join(".bce-tool").join("index.bin");
...
fn test_save_index_no_temp_file_left() {
    ...
    let bce_dir = temp_dir.path().join(".bce-tool");
    let tmp_path = bce_dir.join("index.bin.tmp");
