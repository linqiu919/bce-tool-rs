...
fn test_get_enhancer_endpoint_all_cases() {
    ...
    // Test legacy env var fallback (BCE_ENHANCER_ENDPOINT)
...
fn test_env_enhancer_endpoint_constant() {
    ...
    assert_eq!(ENV_ENHANCER_ENDPOINT_LEGACY, "BCE_ENHANCER_ENDPOINT");
}
...
fn test_default_model_constants() {
    assert_eq!(DEFAULT_CLAUDE_MODEL, "claude-sonnet-5");
    assert_eq!(DEFAULT_OPENAI_MODEL, "gpt-5.6-luna");
    assert_eq!(DEFAULT_GEMINI_MODEL, "gemini-3.7-flash");
