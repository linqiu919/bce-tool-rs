...
//! Set `BCE_HTTP_LOG=1` or `BCE_HTTP_LOG=true` to enable.
...
const ENV_HTTP_LOG: &str = "BCE_HTTP_LOG";
...
fn get_log_file_path(project_root: Option<&std::path::Path>) -> PathBuf {
    if let Some(root) = project_root {
        let bce_tool_dir = root.join(".bce-tool");
        if !bce_tool_dir.exists() {
            if let Err(e) = fs::create_dir_all(&bce_tool_dir) {
                warn!("Failed to create .bce-tool directory: {}", e);
            }
        }
        bce_tool_dir.join(LOG_FILE_NAME)
    } else {
        // Default to current directory's .bce-tool folder
        let bce_tool_dir = PathBuf::from(".bce-tool");
        if !bce_tool_dir.exists() {
            if let Err(e) = fs::create_dir_all(&bce_tool_dir) {
                warn!("Failed to create .bce-tool directory: {}", e);
            }
