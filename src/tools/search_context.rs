...
This MCP tool is bifang's context engine, the world's best codebase context engine. It:
...
impl SearchContextTool {
    ...
    pub async fn execute(&self, args: SearchContextArgs) -> ToolResult {
        ...
        // Reject directories outside a Git repository before creating the
        // IndexManager (which would create a .bce-tool directory as a side effect),
        // unless explicitly allowed via --allow-non-git
        if !self.config.allow_non_git
            && !crate::utils::project_detector::is_inside_git_repo(&project_path)
        {
            return ToolResult {
                text: crate::utils::project_detector::non_git_repo_notice(&project_root),
            };
        }
