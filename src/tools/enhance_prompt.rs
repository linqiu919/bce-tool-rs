...
impl EnhancePromptTool {
    ...
    pub async fn execute(&self, args: EnhancePromptArgs) -> ToolResult {
        ...
        if let Some(ref root) = project_root {
            ...
            // Same restriction as search_context: refuse non-Git directories
            // and tell the AI model why the tool cannot be used here,
            // unless explicitly allowed via --allow-non-git
            if !self.config.allow_non_git
                && root.is_dir()
                && !crate::utils::project_detector::is_inside_git_repo(root)
            {
                return ToolResult {
                    text: crate::utils::project_detector::non_git_repo_notice(
                        &root.display().to_string(),
                    ),
                };
            }

            // Same restriction as indexing: refuse projects whose indexable
            // content exceeds the server-side size limit
            if root.is_dir() {
                match crate::index::IndexManager::new(self.config.clone(), root.clone()) {
                    Ok(manager) => {
                        let total_bytes = manager.indexable_size().await;
                        if total_bytes > crate::index::MAX_PROJECT_BYTES {
                            return ToolResult {
                                text: crate::index::project_too_large_notice(root, total_bytes),
                            };
                        }
                    }
                    Err(e) => {
                        error!("Failed to create IndexManager for size check: {}", e);
                    }
                }
            }
        }
