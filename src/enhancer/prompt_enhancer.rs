...
//! (with `BCE_ENHANCER_ENDPOINT` as a backward-compatible fallback):
...
pub const ENV_ENHANCER_ENDPOINT_LEGACY: &str = "BCE_ENHANCER_ENDPOINT";
...
///
/// Checks `PROMPT_ENHANCER_ENDPOINT` first, then falls back to `BCE_ENHANCER_ENDPOINT`
...
async fn maybe_inject_search_context(
    ...
    if config.base_url.trim().is_empty() || config.token.trim().is_empty() {
        return Err(anyhow!(
            "{} requires BCE search configuration (--base-url and --token) for '{}' endpoint",
            ENV_ENHANCER_INCLUDE_SEARCH_CONTEXT,
            endpoint
        ));
    }
...
mod tests {
    ...
    fn test_maybe_inject_search_context_requires_search_config() {
        ...
        assert!(err
            .to_string()
            .contains("requires BCE search configuration"));
