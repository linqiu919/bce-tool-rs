...
/// Maximum total size of indexable project files (250MB).
...
/// and can explain it to the user instead of retrying blindly.
pub fn project_too_large_notice(path: &Path, total_bytes: u64) -> String {
    format!(
        "TOOL UNAVAILABLE: project at '{}' is too large to index: {:.1} MB of indexable files \
         exceeds the {} MB server-side limit.\n\
         Index upload has been skipped because the backend rejects projects over this size.\n\
         Please inform the user: this project exceeds the size limit, so codebase search and \
         prompt enhancement cannot be used here.\n\
         Suggested fixes: exclude large or generated directories (build artifacts, datasets, \
         vendored dependencies) via .gitignore or .bceignore in the project root, then retry.\n\
         Do not retry this tool until the indexable size has been reduced.",
        path.display(),
...
// v3: dropped the project_name field (now reported per-search instead of
// persisted). The index is bincode (not self-describing), so any layout
// change MUST bump this version — stale caches would otherwise deserialize
// misaligned.
...
/// Index data structure (v3 format: mtime support, no persisted project name)
...
impl IndexManager {
    pub fn new(config: Arc<Config>, project_root: PathBuf) -> Result<Self> {
        ...
        Ok(Self {
            ...
            allow_non_git: config.allow_non_git,
    ...
    /// Total size in bytes of all files that would be indexed
    /// (applies the same exclude / text-file filters as indexing)
    pub async fn indexable_size(&self) -> u64 {
    ...
    pub async fn index_project(&self) -> IndexResult {
        ...
        // Refuse to index directories outside a Git repository to avoid
        // accidentally indexing non-project directories (e.g. Downloads),
        // unless explicitly allowed via --allow-non-git
        if !self.allow_non_git && !is_inside_git_repo(&self.project_root) {
            warn!(
                "Refusing to index non-git directory: {:?}",
                self.project_root
            );
            return IndexResult {
                ...
                message: format!(
                    "Project directory is not inside a Git repository: {}. \
                     Indexing is only allowed for Git repositories to avoid indexing \
                     non-project directories (e.g. Downloads). If this is a real project, \
                     run 'git init' in its root directory first.",
        ...
        let (file_paths, total_bytes) = tokio::task::spawn_blocking(move || {
            let paths = collect_file_paths_standalone(
                &project_root_scan,
        ...
        // Refuse to upload projects whose indexable content exceeds the
        // server-side size limit
        if total_bytes > MAX_PROJECT_BYTES {
            warn!(
                "Project too large to index: {} bytes (limit {} bytes): {:?}",
                total_bytes, MAX_PROJECT_BYTES, self.project_root
            );
            return IndexResult {
                status: "error".to_string(),
                message: project_too_large_notice(&self.project_root, total_bytes),
                stats: None,
            };
        }

        info!(
            "Found {} files to process ({} bytes)",
            file_paths.len(),
            total_bytes
        );
        ...
        // Blob hashes per newly processed file, used to detect files whose
        // upload (partially) failed so they are not recorded as indexed
        let mut new_file_hashes: Vec<(String, Vec<String>)> = Vec::new();
        ...
        for pf in results {
            match pf.result {
                ...
                ProcessedResult::New { blobs, entry } => {
                    new_file_hashes.push((pf.rel_path.clone(), entry.blob_hashes.clone()));
                    new_index.entries.insert(pf.rel_path, entry);
        ...
        // Step 6: Files whose blobs did not all reach the server must NOT be
        // recorded as indexed. Dropping their entries makes the next round's
        // mtime check miss, so they are re-processed and re-uploaded
        // automatically. Otherwise the mtime cache would treat them as
        // uploaded forever and the server would silently miss their content.
        if failed_batch_count > 0 {
            let uploaded: HashSet<&String> = uploaded_blob_names.iter().collect();
            let mut dropped = 0usize;
            for (rel_path, hashes) in &new_file_hashes {
                if !hashes.iter().all(|hash| uploaded.contains(hash)) {
                    new_index.entries.remove(rel_path);
                    dropped += 1;
                }
            }
            if dropped > 0 {
                warn!(
                    "{} files failed to upload; they will be re-uploaded on the next indexing round",
                    dropped
                );
            }
        }

        // Step 7: Save new index (atomic write)
...
fn build_ignore_rules(project_root: &Path) -> Option<Gitignore> {
    let ignore_files = [".gitignore", ".bceignore"];
