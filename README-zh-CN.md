...
bce-tool 是一个 Rust 实现的代码库上下文引擎，使 AI 助手能够使用自然语言查询来搜索和理解代码库。它提供：
...
- **智能排除** - 遵循 `.gitignore`、`.bceignore` 和常见的忽略模式
...
# 二进制文件位于 target/release/bce-tool
```
...
| `--allow-non-git` | 允许索引不在 Git 仓库内的目录。默认情况下索引/检索/增强会拒绝非 Git 目录，以避免误索引个人文件夹（如下载目录） |
...
| `--search <query>` | 一次性检索模式：索引当前目录，执行代码库检索，把结果输出到 stdout 后退出（不启动 MCP 服务器）。适用于脚本和 Agent Skill 调用 |
...
| `PROMPT_ENHANCER_ENDPOINT` | 端点选择：`new`（默认）、`old`、`claude`、`openai`、`gemini` 或 `codex`（同时支持 `BCE_ENHANCER_ENDPOINT` 作为向后兼容） |
...
- 启动 bce-tool 时加上 `--no-webbrowser-enhance-prompt`。
...
该工具支持多个后端端点，通过 `PROMPT_ENHANCER_ENDPOINT` 环境变量控制（同时支持 `BCE_ENHANCER_ENDPOINT` 作为向后兼容）：
...
export PROMPT_ENHANCER_BASE_URL=https://api.anthropic.com
export PROMPT_ENHANCER_TOKEN=your-anthropic-api-key
bce-tool --enhance-prompt "添加用户认证功能"

# 如果还想在第三方增强前注入 search_context，
# 需要同时提供 BCE 检索用的 --base-url 和 --token
...
bce-tool --enhance-prompt "重构认证逻辑"
```
...
您可以在项目根目录创建 `.bceignore` 文件来自定义文件过滤规则，语法与 `.gitignore` 相同：
...
`.gitignore` 和 `.bceignore` 的规则会合并使用，冲突时 `.bceignore` 优先。
...
- 仅处理根目录的 `.gitignore` 和 `.bceignore` 文件（不支持嵌套的忽略文件）
- 需要网络访问索引 API
- 最大文件大小：每个文件 128KB
- 最大批次大小：每次上传批次 1MB
...
本项目是 [missdeer](https://github.com/missdeer) 的 [ace-tool-rs](https://github.com/missdeer/ace-tool-rs) 的衍生项目，适配为 [Better Context Engine](https://github.com/linqiu919/better-context-engine) 的 `bce-tool` 客户端，沿用上游的双许可条款：

- **非商业 / 个人用途** —— [GPL v3](LICENSE)
