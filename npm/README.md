# bce-tool npm shim (Legacy)

> **Note**: This directory contains the legacy "download-on-run" shim which is now marked as `private: true`.
> The actual published package is in `npm/bce-tool/` which uses the modern `optionalDependencies` approach.

This npm package provides a convenient way to install and run `bce-tool` via npm/npx.
...
| Platform | Cache Path |
|----------|------------|
| Windows  | `%LOCALAPPDATA%\bce-tool\<version>\` |
| macOS    | `~/Library/Caches/bce-tool/<version>/` |
| Linux    | `$XDG_CACHE_HOME/bce-tool/<version>/` or `~/.cache/bce-tool/<version>/` |
...
1. **Manual download**: Download the appropriate binary from [GitHub Releases](https://github.com/linqiu919/bce-tool-rs/releases) and place it in the cache directory.
...
This package is part of [bce-tool](https://github.com/linqiu919/bce-tool-rs) and is licensed under GPL-3.0.
