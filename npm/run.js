...
const PACKAGE_NAME = "bce-tool";
const REPO_OWNER = "linqiu919";
const REPO_NAME = "bce-tool-rs";
...
function getAssetName() {
  ...
  switch (platform) {
    case "darwin":
      ...
      return "bce-tool_Darwin_universal.tar.gz";
    case "linux":
      if (arch === "x64") {
        return "bce-tool_Linux_x86_64.tar.gz";
      } else if (arch === "arm64") {
        return "bce-tool_Linux_aarch64.tar.gz";
      }
      throw new Error(
    ...
    case "win32":
      if (arch === "x64") {
        return "bce-tool_Windows_x86_64.zip";
      } else if (arch === "arm64") {
        return "bce-tool_Windows_aarch64.zip";
      }
      throw new Error(
...
async function downloadAndExtract(cacheDir) {
  ...
  try {
    ...
    console.error(
      "  1. Download from https://github.com/linqiu919/bce-tool-rs/releases"
    );
    console.error(`  2. Place binary at: ${binaryPath}`);
    console.error("");
    console.error("Or install via cargo:");
    console.error("  cargo install bce-tool");
    process.exit(1);
  } finally {
